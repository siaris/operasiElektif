...
<form class="form-horizontal well" role="form" method="post" id="form_report">
	<div class="form-group">
        ...
        <div class="col-sm-1-half">
			<input type="text" value="<?php echo (isset($_POST['tanggal_input_sd']) ? $_POST['tanggal_input_sd'] : date($date_format_in_field) ); ?>" name="tanggal_input_sd" class="form-control datepicker required"  placeholder="Sampai" />
		</div>
	</div>
	<div class="form-group">
		<label class="col-sm-1 control-label">Asal Poli </label>
		<div class="col-sm-3">
			<?php echo myform_dropdown('poli_asal', $ddpoli, isset($_POST['poli_asal'])?$_POST['poli_asal']:null, 'class="input-large"'); ?>
		</div>
	</div>
	<div class="form-group">
		...
		<td>
			<table width="100%" style="font-size:12px;" class="table table-bordered table-condensed">
				...
				<tr>
                    ...
                    <td><?= str_replace('<br />',";",$row['rute_desc'])?></td>
					<td><?= $row['desc_status']?></td>
					<td><?= $row['diagnosa']?></td>
					<td><?= $row['tindakan']?></td>
                    <td><?= display_date(json_decode($row['catatan_pra_operasi'],true)['created_date'],'d-m-Y')?></td>
					...
					<td><?= $row['poli_asal']=='300'?'Ruang Inap':$row['nama_poli']?></td>
				</tr>
			<?$i+=1;}?>
			</table>
		</td>
